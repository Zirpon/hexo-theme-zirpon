...
+   [C++ 11是什么，C++ 11标准的由来](#C_11C_11_2)
+   [C++ auto类型推导完全攻略](#C_auto_37)
+   +   [auto 类型推导的语法和规则](#auto__43)
    +   [auto 的高级用法](#auto__83)
    +   [auto 的限制](#auto__126)
    +   [auto 的应用](#auto__156)
    +   +   +   [使用 auto 定义迭代器](#_auto__160)
            +   [auto 用于泛型编程](#auto__192)
+   [C++ decltype类型推导完全攻略](#C_decltype_262)
+   +   +   +   [exp 注意事项](#exp__283)
    +   [decltype 推导规则](#decltype__298)
    +   [decltype 的实际应用](#decltype__379)
+   [汇总auto和decltype的区别](#autodecltype_426)
+   +   [语法格式的区别](#_430)
    +   [对 cv 限定符的处理](#_cv__462)
    +   [对引用的处理](#_501)
    +   [总结](#_531)
+   [C++返回值类型后置（跟踪返回值类型）](#C_547)
+   [C++11对模板实例化中连续右尖括号>>的改进](#C11_627)
+   +   +   +   [扩展阅读](#_686)
+   [C++11使用using定义别名（替代typedef）](#C11usingtypedef_692)
+   [C++11支持函数模板的默认模板参数](#C11_803)
+   [C++11在函数模板和类模板中使用可变参数](#C11_878)
+   +   [可变参数模板](#_919)
    +   +   +   [1) 可变参数函数模板](#1__923)
            +   [2) 可变参数类模板](#2__1020)
+   [C++11 tuple元组详解](#C11_tuple_1076)
+   +   [tuple对象的创建](#tuple_1084)
    +   +   +   [1) 类的构造函数](#1__1095)
            +   [2) make\_tuple()函数](#2_make_tuple_1141)
    +   [tuple常用函数](#tuple_1155)
+   [C++11列表初始化（统一了初始化方式）](#C11_1213)
+   +   [统一的初始化](#_1247)
+   [C++11 lambda匿名函数用法详解](#C11_lambda_1341)
+   +   [lambda匿名函数的定义](#lambda_1347)
    +   +   +   [lambda匿名函数中的\[外部变量\]](#lambda_1392)
+   [C++11非受限联合体（union）](#C11union_1602)
+   +   +   +   [1\. C++11 允许非 POD 类型](#1_C11__POD__1631)
            +   [2\. C++11 允许联合体有静态成员](#2_C11__1670)
    +   [非受限联合体的赋值注意事项](#_1685)
    +   +   +   [placement new 是什么？](#placement_new__1726)
    +   [非受限联合体的匿名声明和“枚举式类”](#_1738)
+   [C++11 for循环（基于范围的循环）详解](#C11_for_1789)
+   [C++11 for循环使用注意事项](#C11_for_1924)
+   [C++11 constexpr：验证是否为常量表达式（长篇神文）](#C11_constexpr_2129)
+   +   [constexpr修饰普通变量](#constexpr_2159)
    +   [constexpr修饰函数](#constexpr_2191)
    +   [constexpr修饰类的构造函数](#constexpr_2295)
    +   [constexpr修饰模板函数](#constexpr_2386)
+   [C++11 constexpr和const的区别](#C11_constexprconst_2431)
+   [C++11 long long超长整形详解](#C11_long_long_2519)
+   [C++11右值引用（一看即懂）](#C11_2596)
+   +   [C++左值和右值](#C_2602)
    +   [C++右值引用](#C_2629)
+   [C++11移动构造函数的功能和用法](#C11_2700)
+   +   [C++11移动语义是什么](#C11_2704)
    +   [C++移动构造函数（移动语义的具体实现）](#C_2769)
+   [C++11 move()函数：将左值强制转换为右值](#C11_move_2834)
+   [C++11引用限定符的用法](#C11_2945)
+   +   [const和引用限定符](#const_3025)
+   [C++11完美转发及其实现](#C11_3073)
+   [C++11 nullptr：初始化空指针](#C11_nullptr_3205)
+   [C++11 shared\_ptr智能指针（超级详细）](#C11_shared_ptr_3299)
+   +   [C++11 shared\_ptr智能指针](#C11_shared_ptr_3320)
    +   +   +   [1、shared\_ptr智能指针的创建](#1shared_ptr_3333)
            +   [2、shared\_ptr模板类提供的成员方法](#2shared_ptrT_3410)
+   [C++11 unique\_ptr智能指针](#C11_unique_ptr_3466)
+   +   [unique\_ptr智能指针的创建](#unique_ptr_3483)
    +   [unique\_ptr模板类提供的成员方法](#unique_ptrT_3530)
+   [C++11 weak\_ptr智能指针](#C11_weak_ptr_3586)
+   +   [C++11 weak\_ptr智能指针](#C11_weak_ptr_3593)
    +   +   +   [1、weak\_ptr指针的创建](#1weak_ptr_3610)
            +   [2) weak\_ptr模板类提供的成员方法](#2_weak_ptr_3637)
+   [C++14 新特性总结](#C14__3685)
+   +   +   +   [函数返回值类型推导](#_3689)
            +   [lambda参数auto](#lambdaauto_3786)
            +   [变量模板](#_3802)
            +   [别名模板](#_3817)
            +   [constexpr的限制](#constexpr_3841)
            +   [\[\[\`deprecated\`\]\]标记](#deprecated_3882)
            +   [二进制字面量与整形字面量分隔符](#_3907)
            +   [std::make\_unique](#stdmake_unique_3916)
            +   [std::shared\_timed\_mutex与std::shared\_lock](#stdshared_timed_mutexstdshared_lock_3925)
            +   [std::integer\_sequence](#stdinteger_sequence_3954)
            +   [std::exchange](#stdexchange_3992)
            +   [std::quoted](#stdquoted_4023)
+   [C++17新特性总结](#C17_4045)
+   +   +   +   [构造函数模板推导](#_4072)
            +   [结构化绑定](#_4087)
            +   [if-switch语句初始化](#ifswitch_4198)
            +   [内联变量](#_4226)
            +   [折叠表达式](#_4243)
            +   [constexpr lambda表达式](#constexpr_lambda_4258)
            +   [namespace嵌套](#namespace_4273)
            +   [\_\_has\_include预处理表达式](#__has_include_4290)
            +   [在lambda表达式用\*this捕获对象副本](#lambdathis_4316)
            +   [新增Attribute](#Attribute_4356)
            +   [字符串转换](#_4418)
            +   [std::variant](#stdvariant_4447)
        +   [std::optional](#stdoptional_4489)
        +   +   [std::any](#stdany_4525)
            +   [std::apply](#stdapply_4548)
            +   [std::make\_from\_tuple](#stdmake_from_tuple_4564)
            +   [std::string\_view](#stdstring_view_4580)
            +   [as\_const](#as_const_4598)
            +   [file\_system](#file_system_4607)
            +   [std::shared\_mutex](#stdshared_mutex_4619)
+   [C++20 新增特性总结](#C20__4623)
+   +   [新增关键字(keywords)](#keywords_4698)
    +   [新增标识符(Identifies)](#Identifies_4709)
    +   [模块(Modules)](#Modules_4714)
    +   +   [优点](#_4716)
        +   [创建模块](#_4727)
        +   [引用模块](#_4738)
        +   [import 头文件](#import__4748)
    +   [Ranges](#Ranges_4755)
    +   +   [例子](#_4782)
    +   [协程(Coroutines)](#Coroutines_4822)
    +   +   [什么是协程](#_4824)
        +   [例子(VC++)](#VC_4848)
    +   [Concepts](#Concepts_4868)
    +   +   [如何定义](#_4874)
        +   [使用](#_4880)
        +   [例子](#_4895)
    +   [Lambda 表达式的更新](#Lambda__4919)
    +   +   [\[=, this\] 需要显式捕获\`this\`变量](#_this_this_4921)
        +   [模板形式的 Lambda 表达式](#_Lambda__4926)
        +   [Lambda 表达式捕获支持打包展开(Pack Expansion)](#Lambda_Pack_Expansion_4993)
    +   [常量表达式(\`constexpr\`) 的更新](#constexpr__5018)
    +   +   [\`constexpr\` string & vector](#constexpr_string__vector_5039)
    +   [原子(Atomic)智能指针](#Atomic_5044)
    +   +   [例子](#_5064)
    +   [自动合流(Joining), 可中断(Cancellable) 的线程](#Joining_Cancellable__5105)
    +   +   [例子](#_5136)
    +   [C++20 同步(Synchronization)库](#C20_Synchronization_5173)
    +   +   [std::atomic\_ref](#stdatomic_ref_5228)
    +   [其他更新](#_5234)
    +   +   [指定初始化(Designated Initializers)](#Designated_Initializers_5236)
        +   [航天飞机操作符 <=>](#__5246)
        +   [范围 for 循环语句支持初始化语句](#_for__5309)
        +   [非类型模板形参支持字符串](#_5364)
        +   [\[\[likely\]\], \[\[unlikely\]\]](#likely_unlikely_5375)
        +   [日历(Calendar)和时区(Timezone)功能](#CalendarTimezone_5387)
        +   [std::span](#stdspan_5471)
        +   [特性测试宏](#_5487)
        +   [](#version_5504)
        +   [\`consteval\` 函数](#consteval__5508)
        +   [\`constinit\`](#constinit_5512)
        +   [用 \`using\` 引用 \`enum\` 类型](#_using__enum__5529)
        +   [格式化库(\`std::format\`)](#stdformat_5561)
        +   [增加数学常量](#_5569)
        +   [std::source\_location](#stdsource_location_5576)
        +   [\[\[nodiscard(reason)\]\]](#nodiscardreason_5580)
        +   [位运算](#_5589)
        +   [一些小更新](#_5593)
